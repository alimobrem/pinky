...
class TestPodHealthScanner:
    ...
    async def test_crashloop_detected(self, scanner: Definition) -> None:
        pods = [
            {
                "name": "web-abc",
                "namespace": "prod",
    ...
    async def test_healthy_pod_no_observations(self, scanner: Definition) -> None:
        pods = [
            {
                "name": "healthy",
                "namespace": "prod",
    ...
    async def test_excessive_restarts(self, scanner: Definition) -> None:
        pods = [
            {
                "name": "flaky",
                "namespace": "staging",
    ...
    async def test_image_pull_backoff(self, scanner: Definition) -> None:
        pods = [
            {
                "name": "bad-image",
                "namespace": "dev",
    ...
    async def test_pending_too_long(self, scanner: Definition) -> None:
    ...
    async def test_pending_recently_created_no_observation(self, scanner: Definition) -> None:
...
class TestNodeConditionsScanner:
    ...
    async def test_memory_pressure(self, scanner: Definition) -> None:
        nodes = [
            {
                "name": "node-1",
                "unschedulable": False,
        ...
        obs = await run_generic_checks(nodes, CLUSTER, scanner)
        mp = [o for o in obs if o.check_id == "memory-pressure"]
        assert len(mp) == 1
        assert mp[0].severity == "high"

    async def test_not_ready(self, scanner: Definition) -> None:
        nodes = [
            {
                "name": "node-2",
                "unschedulable": False,
    ...
    async def test_unschedulable(self, scanner: Definition) -> None:
        nodes = [
            {
                "name": "node-3",
                "unschedulable": True,
    ...
    async def test_healthy_node_no_observations(self, scanner: Definition) -> None:
        nodes = [
            {
                "name": "node-ok",
                "unschedulable": False,
...
class TestDeploymentHealthScanner:
    ...
    async def test_rollout_stalled(self, scanner: Definition) -> None:
        deps = [
            {
                "name": "api",
                "namespace": "prod",
        ...
        obs = await run_generic_checks(deps, CLUSTER, scanner)
        stalled = [o for o in obs if o.check_id == "rollout-stalled"]
        assert len(stalled) == 1
        assert stalled[0].severity == "high"

    async def test_unavailable_replicas(self, scanner: Definition) -> None:
        deps = [
            {
                "name": "web",
                "namespace": "prod",
    ...
    async def test_healthy_deployment_no_observations(self, scanner: Definition) -> None:
        deps = [
            {
                "name": "stable",
                "namespace": "prod",
    ...
    async def test_replica_mismatch(self, scanner: Definition) -> None:
...
class TestCertExpiryScanner:
    ...
    async def test_no_cert_data_no_observations(self, scanner: Definition) -> None:
        secrets = [
            {
                "name": "my-tls",
                "namespace": "prod",
        ...
        obs = await run_generic_checks(secrets, CLUSTER, scanner)
        assert obs == []

    async def test_empty_cert_no_observations(self, scanner: Definition) -> None:
        secrets = [
            {
                "name": "empty-tls",
                "namespace": "prod",
...
class TestPvcHealthScanner:
    ...
    async def test_pvc_pending(self, scanner: Definition) -> None:
        pvcs = [
            {"name": "data-vol", "namespace": "prod", "phase": "Pending"},
        ]
        obs = await run_generic_checks(pvcs, CLUSTER, scanner)
        pending = [o for o in obs if o.check_id == "pvc-pending"]
        assert len(pending) == 1
        assert pending[0].severity == "high"

    async def test_pvc_lost(self, scanner: Definition) -> None:
        pvcs = [
    ...
    async def test_bound_pvc_no_observations(self, scanner: Definition) -> None:
        pvcs = [
...
class TestResourceQuotasScanner:
    ...
    async def test_quota_exceeded_with_scalar_paths(self, scanner: Definition) -> None:
        ...
        obs = await run_generic_checks(quotas, CLUSTER, scanner)
        exceeded = [o for o in obs if o.check_id == "quota-exceeded"]
        assert len(exceeded) == 1
        assert exceeded[0].severity == "high"

    async def test_quota_dict_values_do_not_fire(self, scanner: Definition) -> None:
    ...
    async def test_quota_under_limit_no_observations(self, scanner: Definition) -> None:
        quotas = [
            {
                "name": "cpu-quota",
                "namespace": "team-a",
    ...
    async def test_quota_near_limit(self, scanner: Definition) -> None:
        quotas = [
            {
                "name": "cpu-quota",
                "namespace": "team-a",
...
class TestIngressHealthScanner:
    ...
    async def test_ingress_no_backend(self, scanner: Definition) -> None:
        ...
        obs = await run_generic_checks(ingresses, CLUSTER, scanner)
        no_backend = [o for o in obs if o.check_id == "ingress-no-backend"]
        assert len(no_backend) == 1
        assert no_backend[0].severity == "high"

    async def test_ingress_with_backends_no_observations(self, scanner: Definition) -> None:
        ingresses = [
            {
...
class TestStatefulsetHealthScanner:
    ...
    async def test_rollout_stuck(self, scanner: Definition) -> None:
        sts = [
            {
                "name": "db",
                "namespace": "prod",
        ...
        obs = await run_generic_checks(sts, CLUSTER, scanner)
        stuck = [o for o in obs if o.check_id == "sts-rollout-stuck"]
        assert len(stuck) == 1
        assert stuck[0].severity == "high"

    async def test_replicas_unavailable(self, scanner: Definition) -> None:
        sts = [
            {
                "name": "cache",
                "namespace": "prod",
    ...
    async def test_healthy_statefulset(self, scanner: Definition) -> None:
        sts = [
            {
                "name": "db",
                "namespace": "prod",
...
class TestJobHealthScanner:
    ...
    async def test_job_failed(self, scanner: Definition) -> None:
        jobs = [
            {
                "kind": "Job",
                "name": "migration",
        ...
        obs = await run_generic_checks(jobs, CLUSTER, scanner)
        failed = [o for o in obs if o.check_id == "job-failed"]
        assert len(failed) == 1
        assert failed[0].severity == "high"

    async def test_job_deadline_exceeded(self, scanner: Definition) -> None:
        jobs = [
            {
                "kind": "Job",
                "name": "etl",
    ...
    async def test_cronjob_missed_schedule(self, scanner: Definition) -> None:
        ...
        obs = await run_generic_checks(cronjobs, CLUSTER, scanner)
        missed = [o for o in obs if o.check_id == "cronjob-missed"]
        assert len(missed) == 1
        assert missed[0].severity == "medium"

    async def test_cronjob_recent_schedule_no_observation(self, scanner: Definition) -> None:
    ...
    async def test_healthy_job_no_observations(self, scanner: Definition) -> None:
        jobs = [
            {
                "kind": "Job",
                "name": "seed",
...
class TestServiceEndpointsScanner:
    ...
    async def test_service_no_endpoints(self, scanner: Definition) -> None:
        services = [
            {
                "name": "api",
                "namespace": "prod",
        ...
        obs = await run_generic_checks(services, CLUSTER, scanner)
        no_ep = [o for o in obs if o.check_id == "service-no-endpoints"]
        assert len(no_ep) == 1
        assert no_ep[0].severity == "high"

    async def test_service_without_selector_no_observations(self, scanner: Definition) -> None:
    ...
    async def test_service_with_endpoints_no_observations(self, scanner: Definition) -> None:
        services = [
            {
                "name": "web",
                "namespace": "prod",
...
class TestDaemonsetHealthScanner:
    ...
    async def test_daemonset_unavailable(self, scanner: Definition) -> None:
        dsets = [
            {
                "name": "fluentd",
                "namespace": "logging",
        ...
        obs = await run_generic_checks(dsets, CLUSTER, scanner)
        unavail = [o for o in obs if o.check_id == "daemonset-unavailable"]
        assert len(unavail) == 1
        assert unavail[0].severity == "high"

    async def test_healthy_daemonset_no_observations(self, scanner: Definition) -> None:
        dsets = [
            {
                "name": "node-exporter",
    ...
    async def test_daemonset_misscheduled(self, scanner: Definition) -> None:
        dsets = [
            {
                "name": "kube-proxy",
    ...
    async def test_desired_mismatch(self, scanner: Definition) -> None:
        dsets = [
            {
                "name": "agent",
                "namespace": "monitoring",
...
class TestPodResourceUsageScanner:
    ...
    async def test_no_prom_client_no_observations(self, scanner: Definition) -> None:
...
class TestResourceLimitsScanner:
    ...
    async def test_no_resource_limits(self, scanner: Definition) -> None:
        pods = [
            {
                "name": "no-limits",
                "namespace": "dev",
    ...
    async def test_has_limits_no_observations(self, scanner: Definition) -> None:
        pods = [
            {
                "name": "with-limits",
                "namespace": "prod",
